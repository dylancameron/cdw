...
Raindrops.prototype = {
	...
	updateRain(timeScale) {
		...
		if (this.options.raining) {
			let limit =
				this.options.rainLimit * timeScale * this.areaMultiplier;
			let count = 0;
			while (
				chance(
					this.options.rainChance * timeScale * this.areaMultiplier
				) &&
				count < limit
			) {
				count++;
	...
	updateDrops(timeScale) {
		...
		this.drops.forEach(function (drop, i) {
			if (!drop.killed) {
				...
				if (this.options.raining) {
					drop.lastSpawn +=
						drop.momentum * timeScale * this.options.trailRate;
				...
				if (!drop.killed) {
					...
					if (moved && this.options.dropletsRate > 0)
						this.clearDroplets(
							...
							drop.y,
							drop.r *
								this.options.dropletsCleaningRadiusMultiplier
						);
